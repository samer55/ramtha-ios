...
componentWillMount(){
  ...
  const fields = orderStore.profile.form.fields;
  this.setState({
...
postContact = async ()=>{
  ...
  const params = {"user_id":orderStore.profile.id,"form_type":"public_profile_contact_form","name":this.state.name,"email":this.state.email,"phone":this.state.phoneNo,"message":this.state.message};
  ...
  render() {
    let {orderStore} = Store;
    const form = orderStore.profile.form;
    const fields = orderStore.profile.form.fields;
      return (
        <View style = {{
          height:'100%',
