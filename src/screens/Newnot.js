  ...
  _pickImage = async () => {
   ...
   onAddItem = (s) => {
    ...
    notification=()=>{
      ...
      firebaseApp.database().ref(`notification/${newPostKey}`).update({
          username:'سوق الرمثا',
          head:this.state.Adtitle,
            body:this.state.Addes,
            createdAt: firebase.database.ServerValue.TIMESTAMP,
          notuid:newPostKey
      });
      fetch('https://onesignal.com/api/v1/notifications',
       {
         ...
         headers: {
           'Accept': 'application/json',
           'Content-Type': 'application/json',
           'Authorization': "Basic NTc3OTM5ZDYtMGI5Mi00ZjMxLWIzZDAtOWYwNTQ2MGY5ZGFk",
         },
         body: JSON.stringify(
         {
           app_id: "2d2cde7a-e181-4deb-81c6-fd84f70992b9",
           included_segments: ["All"],
           headings: {"en": this.state.Adtitle},
           android_sound: "fishing",
           data: {"puid": newPostKey, "new_message":true},
           ios_sound: "fishing.caf",
           contents: {"en": this.state.Addes},
  //  filters: [{"field":"tag","key":"uid","relation":"=","value":userid}],
         })
       })
   }
    ...
    return (
      ...
      <Spinner
         ...
         onPress={this.notification}
           ...
           >
             <Text style={[gStyle.button,gStyle.text['dark']]}>{'ارسل'}</Text>
           </Button>
...
return (
  ...
  <Form style={{width:width-10 }}>
              ...
              <Textarea rowSpan={5} style={[gStyle.text[theme],{backgroundColor: '#ffffff'}]} value={this.state.Addes} onChangeText={(Addes)=>this.setState({Addes})} bordered placeholder="اضف تفاصيل" />
