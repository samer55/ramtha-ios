  ...
  DisplayPosts = (d) => {
    ...
    firebaseApp.database().ref(`Ads`).on('value', (snapshot) => {
         ...
         snapshot.forEach((child) => {
           if (child.val().likedby) {
            let userEmail = firebaseApp.auth().currentUser.email;

             let  liked = child.val().likedby ? Object.values(child.val().likedby).map((project)=>{
...
<ShowScroller dataset={this.state.posts} navigation={this.props.navigation} type="latest" cats="fav"/>
