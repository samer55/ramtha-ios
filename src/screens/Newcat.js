...
import { Alert ,Dimensions,ScrollView,View,StyleSheet,TouchableOpacity,TouchableHighlight,Image,PixelRatio,ImageBackground} from "react-native";
 ...
 handleClose = () => {
  ...
  if (this.state.Adtitle.length >0) {
          ...
          const postData1 = {
            ...
            name:this.state.Adtitle,
            id:newPostKey,
indsec:this.state.ind1
          }
          let updates = {}
          let updatess = {}

          updates['list/' +this.state.ind+'/secdata/'+newPostKey] = postData1
...
onPress={this.create}
  ...
  >
    <Text style={[gStyle.button,gStyle.text['dark']]}>اضف</Text>
  </Button>
</View>:null}
...
{this.state.typeofservice!=="null"?  <Item picker styke={{marginVertical:10,alignSelf:'center',alignItems:'center'}} >
  <Picker

  style={{ height: 50, width: width-20,alignSelf: 'center',marginVertical: 10}}
  selectedValue={this.state.typeofservices}
  onValueChange={(d,i)=>this.setState({typeofservices:d,ind1:i})}

  >
{offers[this.state.ind].section.map((item)=>{
  return(
  ...
  <Input inputStyle={{backgroundColor: '#ffffff',padding: 10,borderWidth: 0.4,borderColor: '#dddddd'}} containerStyle={{borderRadius: 12}}
    placeholder='اضف فرع..' onChangeText={(Adtitle)=>this.setState({Adtitle})} value={this.state.Adtitle}
  />
</Form>
          </View>
)
...
return (
  ...
  style={{backgroundColor: '#fdfdfd',height: 50,justifyContent: 'center',alignItems: 'center',marginHorizontal: 10,paddingHorizontal: 20}}
  onChangeText={text => {
    this.setState({
      advanced: text
    })
...
<TextInputMask
  ...
  style={{height: 50,backgroundColor: '#ffffff',paddingHorizontal: 20,marginHorizontal: 10}}
...
<Label style={{fontFamily: 'Cairo-Regular',marginVertical: 10}}>
بريد الكتروني (اختياري)</Label>

        <Input inputStyle={{backgroundColor: '#ffffff',padding: 10,borderWidth: 0.4,borderColor: '#dddddd',marginVertical: 3}} containerStyle={{borderRadius: 12}}
          placeholder="مثال :user@gmail.com" value={this.state.Admail} onChangeText={(Admail)=>this.setState({Admail})}
        />
...
{this.state.region.longitude>0?  <Text style={[gStyle.button,{color:'#800020'}]}>{this.state.region.longitude}{this.state.region.latitude}</Text>:null}
  ...
  >
    <Text style={[gStyle.button,{color:'#fff'}]}>{'اختر موقع الاعلان'}</Text>
                    ...
                    <Picker.Item label="اربد" value="open" />
                    <Picker.Item label="البلقاء" value="close" />
                    <Picker.Item label="جرش" value="dateopposite" />
                    <Picker.Item label="الزرقاء" value="datedopposite" />
                    <Picker.Item label="الطفيلة" value="datefsopposite" />
                    <Picker.Item label="عجلون" value="dateogpposite" />
                    <Picker.Item label="العقبة" value="datweopposite" />
                    <Picker.Item label="عمان" value="datesopposite" />
                    <Picker.Item label="الكرك" value="dategopposite" />
                    <Picker.Item label="مادبا" value="dateoapposite" />
                    <Picker.Item label="معان" value="dateopfgwposite" />
                    <Picker.Item label="الكرك" value="dateosfspposite" />
