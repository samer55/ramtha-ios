...
import { Alert ,Dimensions,ScrollView,View,StyleSheet,TouchableOpacity,TouchableHighlight,Image,PixelRatio,Platform,ImageBackground,ActivityIndicator} from "react-native";
...
import {  Picker as P } from "react-native";
...
name:'عقارات واراضي',
type:'web-design',
section:["منازل وشقق","اراضي ومزارع","محلات تجارية","شقق مفروشة","اخرى"]
  },
  {
id:24,
...
thirdlie:'',
  ...
  {this.state.data.length!==0?  this.setState({
                    placeHolderText: "Select industry",
                    selectedText: this.state.data.section,
                         title:this.state.data.title,
                         typeofservice:this.state.data.typeofservice,
                         typeofservices:this.state.data.hsection,
advanced:this.state.data.price,
                         Adwhats:this.state.data.Adwhats,
                         Admail:'',
                         international:this.state.data.Adphone,
               list:this.state.data.img?Object.values(this.state.data.img):[],
               used:this.state.data.used,
                 Addes:this.state.data.about,
                 city:this.state.data.city,
                 Adtitle:this.state.data.title,
...
send=()=>{
 ...
 handleClose = () => {
  ...
  create = () => {
    ...
    const newPostKey = firebaseApp.database().ref('Ads').push().key




  if (this.state.Adtitle.length >0) {
  if (this.state.typeofservice !=='null') {
  if (this.state.typeofservices.length >0) {
  if (this.state.selectedText.length >0) {
      if (this.state.Addes.length >0) {
        if (this.state.international !== '+962'&&this.state.international.length>0) {
          const postData = {
            writerId:firebaseApp.auth().currentUser.uid,
            title:this.state.Adtitle,
            user:firebaseApp.auth().currentUser.displayName,
          typeofservice:this.state.typeofservice,
          hsection:this.state.typeofservices,
          section:this.state.selectedText,
          city:this.state.city,
        about:this.state.Addes,
        used:this.state.used,
        Adphone:this.state.international,
        img:this.state.list,
        region:this.state.region,
        Adwhats:this.state.Adwhats,
        Admail:this.state.Admail,
          ...
          updates['Ads/' +newPostKey] = postData
        ...
        .catch(error => {
          ...
          this.setState({ploading:false,  postStatus: 'الرجاء اضافة رقم هاتف',shows:true ,pcolor:'#800020'})
          setTimeout(() => {
          this.setState({shows:false})
        }, 3000)
      }
        ...
        this.setState({ploading:false,  postStatus: 'الرجاء اضافة تفاصيل او شرح للاعلان',shows:true ,pcolor:'#800020'})
        setTimeout(() => {
        this.setState({shows:false})
      }, 3000)
    }
          ...
          this.setState({ploading:false,  postStatus: 'الرجاء اختيار فرع من القسم',shows:true ,pcolor:'#800020'})
          setTimeout(() => {
          this.setState({shows:false})
        }, 3000)
      }
          ...
          this.setState({ploading:false,  postStatus: 'الرجاء اختيار فرع',shows:true ,pcolor:'#800020'})
          setTimeout(() => {
          this.setState({shows:false})
        }, 3000)
      }
        ...
        this.setState({ploading:false,  postStatus: 'الرجاء اختيار قسم',shows:true ,pcolor:'#800020'})
        setTimeout(() => {
        this.setState({shows:false})
      }, 3000)
    }
      ...
      if (this.state.Addes.length >0) {
        ...
        Admail:this.state.Admail,
          ...
          accept:false,
            ...
            updatedAt: firebase.database.ServerValue.TIMESTAMP,
            postuid:this.state.data.postuid,

          }
          let updates = {}
          let updatess = {}

          updates['Ads/' +this.state.data.postuid] = postData
          firebaseApp.database().ref().update(updates)
          .then(() => {
          { /* fetch('https://onesignal.com/api/v1/notifications',
             {
    ...
    return (
      <ThemeContext.Consumer>
        {theme => (
          ...
          {this.state.login?
          <View style={{flex:1}}>
{this.state.status==='جاري تحميل الصورة'?<Text style={{color: 'red'}}>{this.state.status}</Text>:null }
      ...
      <Spinner
           ...
           {this.state.page==1?this.Adddare(theme):null}
           ...
           <Button
             block
         onPress={this.state.page==1?(this.state.data.length===0?this.create:this.Editads):()=>{}}
             style={{ justifyContent: 'center', alignItems: 'center',padding: 15,flex:1,marginHorizontal: 5,backgroundColor: "#800020"}}
           >
             <Text style={[gStyle.button,gStyle.text['dark']]}>{this.state.page==1?'اعلن':'التالي'}</Text>
           </Button>
...
<Text style={[gStyle.text[theme],  gStyle.p,{alignSelf: 'center',marginHorizontal: 20,fontSize: 21}]}>{this.state.postStatus}</Text>
      ...
      onPressCloseButton={this.closePanel}
           >
      <Mappickers region={(d)=>this.setState({region:d})} onPressclose={this.closePanel}/>
    </SwipeablePanel>

      </View>:<View style={{flex:1,justifyContent: 'center',alignItems: 'center'}}>
<Text>الرجاء التسجيل لاضافة اعلان</Text>
      </View>}
        </View>
    )}
...
return (
  ...
  <Input inputStyle={{backgroundColor: '#ffffff',padding: 10,borderWidth: 0.4,borderColor: '#dddddd'}} containerStyle={{borderRadius: 12}}
    placeholder='اضف عنوان او اسم..' placeholderTextColor = "#000" onChangeText={(Adtitle)=>this.setState({Adtitle})} value={this.state.Adtitle}
  />

  </Form>
  <TouchableOpacity onPress={()=>this.setState({se:!this.state.se})} style={{width: width,paddingHorizontal: 20,justifyContent: 'flex-start',alignItems: 'center',height: 50,marginVertical: 10,flexDirection: 'row'}}>
<Text style={{fontFamily: 'Cairo-Bold',marginHorizontal: 5,fontWeight: '500',fontSize: 17}}>{this.state.typeofservice!=='null'?this.state.typeofservice:'الرجاء اختيار القسم المخصص للاعلان'}</Text>
<Icon name="arrow-down" />
  ...
  {this.state.data.length===0&&this.state.se?
  <P

  style={{  flex:1,width,alignSelf: 'center',marginVertical: 10}}
...
{this.state.ind>0 ? <TouchableOpacity onPress={()=>this.setState({se1:!this.state.se1})} style={{width: width,paddingHorizontal: 20,justifyContent: 'flex-start',alignItems: 'center',height: 50,marginVertical: 10,flexDirection: 'row'}}>
<Text style={{fontFamily: 'Cairo-Bold',marginHorizontal: 5,fontWeight: '500',fontSize: 17}}>{this.state.typeofservices!=='جميع الافرع'?this.state.typeofservices:'الرجاء اختيار الفرع'}</Text>
<Icon name="arrow-down" />

  </TouchableOpacity>:null}
{this.state.data.length===0&&this.state.se1&&this.state.typeofservice!=="null"&&  this.state.ind>0&&offers[this.state.ind].section.length>0 ?  <Item picker style={{marginVertical:10,alignSelf:'center',alignItems:'center'}} >
  <P
  placeholder="الرجاء اختيار الفرع"
  iosIcon={<Icon name="arrow-down" />}

  style={{ flex:1,width,alignSelf: 'center',marginVertical: 10}}
  selectedValue={this.state.typeofservices}
  onValueChange={(d,i)=>{
    let dv=i-1
    this.setState({typeofservices:d,ind1:dv});
    this.displayCategories(dv)
  }}

  >
 <Picker.Item label="اختر قسم" value="null" />
{this.state.ind>0&&offers[this.state.ind].section.map((item)=>{
  return(
  ...
  <View style={{borderWidth: 1,borderColor: '#dddddd',marginBottom: 10}}/>
      ...
      {this.state.typeofservice!=="null"&&this.state.Online.length>0?    <RNPicker
                  dataSource={this.state.Online}
                  dummyDataSource={this.state.Online}
                  defaultValue={true}
                  pickerTitle={"اختر فرع"}
                  showSearchBar={true}
                  disablePicker={false}
                  changeAnimation={"none"}
                  searchBarPlaceHolder={"ابحث....."}
                ...
                />:this.state.refreshing?  <View style={ {
                height: 50,width: width,
                justifyContent: 'center',
                alignItems: 'center',

              }}>
                    <ActivityIndicator size='large' color="#eb144c"/>
                  </View>:null}
          </View>
)
}
...
return (
  ...
  style={{backgroundColor: '#fdfdfd',height: 50,justifyContent: 'center',alignItems: 'center',marginHorizontal: 10,paddingHorizontal: 20,color:'black'}}
  onChangeText={text => {
    this.setState({
      advanced: text
    })
...
<Right>
<Switch value={this.state.used} onValueChange={(d)=>this.setState({used:d})}/>
</Right>
</ListItem>
                </Form>
...
<TextInputMask
  ...
  placeholderTextColor = "#000"
  style={{height: 50,backgroundColor: '#ffffff',paddingHorizontal: 20,marginHorizontal: 10,color:'black'}}
...
{this.state.region.longitude>0?  <Text style={[gStyle.button,{color:'#800020'}]}>{this.state.region.longitude}{this.state.region.latitude}</Text>:null}
  ...
  block
    ...
    style={{ justifyContent: 'center', alignItems: 'center',flex:1,marginHorizontal: 5,width: '100%',backgroundColor:'#800020'}}
  >
    <Text style={[gStyle.button,{color:'#fff'}]}>{'اختر موقع الاعلان'}</Text>
  </Button>
  </View>
                      </Form>
