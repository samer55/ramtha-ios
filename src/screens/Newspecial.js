...
send=()=>{
 ...
 handleClose = () => {
  ...
  create = () => {
    ...
    const newPostKey = firebaseApp.database().ref('Special').push().key




  if (this.state.image) {
      if (this.state.timestay >0) {
          const postData = {
            writerId:firebaseApp.auth().currentUser.uid,
            user:firebaseApp.auth().currentUser.displayName,
          place:this.state.typeofservice,
          city:this.state.city,
        img:this.state.image,
          accept:false,
            createdAt: firebase.database.ServerValue.TIMESTAMP,
            updatedAt: new Date().getTime()+this.state.timestay*24*60 *60 *1000,
            postuid:newPostKey,
          ...
          updates['Special/' +this.state.typeofservice+'/'+this.state.city+'/'+newPostKey] = postData
        ...
        this.setState({ploading:false,  postStatus: 'اختر مدة الاعلان',shows:true ,pcolor:'#800020'})
          setTimeout(() => {
          this.setState({shows:false})
        }, 3000)
      }
    ...
    this.setState({ploading:false,  postStatus: 'الرجاء اختيار صور',shows:true ,pcolor:'#800020'})
      setTimeout(() => {
      this.setState({shows:false})
    }, 3000)
  }
...
if (i==child.val().indsec) {
 ...
 _handleImagePicked = async pickerResult => {
     ...
     this.setState({status:'جاري اضافة صورة',refreshing:true})
...
<Text style={{color:'#800020'}}>{this.state.status}</Text>
  <Form style={{width:width-10 }}>
  <Label style={{fontFamily: 'Cairo-Regular',marginVertical: 10}}>
...
{this.state.image?  <ImageBackground source={{uri:this.state.image}} style={{marginHorizontal: 10,borderRadius: 12,width: 150,height: 100,resizeMode: 'contain'}}>
<TouchableOpacity onPress={()=>this.setState({image:null})} style={{position: 'absolute',top: 4,left:4,backgroundColor: 'rgba(0,0,0,0.2)',width: '100%'}}>
  ...
  <Icon name='closecircle' type="AntDesign" style={{color:'#fff'}}/>
                      ...
                      >
                      <Picker.Item label={'شاشة الدخول'} value={'start'} />
                      <Picker.Item label={'شاشة البحث'} value={'search'} />
                      ...
                      style={{ height: 50, width: width-20,alignSelf: 'center',marginVertical: 10}}
                      selectedValue={this.state.timestay}
                      onValueChange={(d,i)=>this.setState({timestay:d})}

                      >
                      <Picker.Item label={'مدة الاعلان'} value={0} />
                      <Picker.Item label={'يوم واحد'} value={1} />
                      <Picker.Item label={'يومين'} value={2} />
                      <Picker.Item label={'ثلاث ايام'} value={3} />
                      <Picker.Item label={'اسبوع'} value={7} />
                      <Picker.Item label={'اسبوعين'} value={14} />
                      <Picker.Item label={'ثلاث اسابيع'} value={21} />
                      <Picker.Item label={'شهر'} value={30} />
                      <Picker.Item label={'شهرين'} value={60} />
